...
{id:'fundamentos',name:'Fundamentos VR',icon:'🥽',color:'#1F66F2',desc:'Presencia, embodiment, affordance, mareo y UI diegética/espacial',qs:[
 {tag:'Affordance',title:'Affordance',
  ...
  mc:['Capacidad de un objeto de comunicar visualmente cómo debe usarse','Sensación de que el cuerpo virtual responde como propio','Coherencia del entorno que hace sentir al usuario "ahí"','Disposición de la UI a distancia cómoda y legible'],correct:0,
 ...
 {tag:'Embodiment',title:'Embodiment',
  ...
  mc:['Que el objeto comunica visualmente cómo debe usarse','Que el cuerpo virtual se siente como propio del usuario','Que la repetición de gestos los vuelve reflejos','Que el cuerpo real se usa como interfaz principal'],correct:1,
...
{id:'corporalidad',name:'Corporalidad y Memoria Muscular',icon:'🤲',color:'#2FB57A',desc:'El cuerpo como interfaz principal y la automatización por repetición',qs:[
 {tag:'Corporalidad',title:'Corporalidad',
  ...
  mc:['Sensación de que el cuerpo virtual pertenece al usuario','Utilización del propio cuerpo como herramienta principal de interacción','Automatización de los movimientos por repetición','Conjunto de consecuencias visibles que produce cada acción'],correct:1,
 ...
 {tag:'Memoria muscular',title:'Memoria muscular',
  ...
  mc:['Sensación de que el cuerpo virtual pertenece al usuario','Diseño de las acciones físicas que ejecuta el jugador','Automatización de movimientos por repetición hasta hacerlos reflejos','Calidad de las sensaciones transmitidas durante la interacción'],correct:2,
...
{id:'interacciones',name:'Interacciones Meta SDK',icon:'🤜',color:'#E8822E',desc:'Interactor/Interactable, Grab, Snap completo, Event Wrappers completos, poses y gizmos',qs:[
 ...
 {tag:'Hand Grab Pose',title:'HandGrabPose y Fingers Freedom',
  ...
  mc:['El dedo queda fijo en la pose definida','El dedo sigue libremente el hand tracking real','El dedo se mueve dentro de un rango limitado','El dedo se excluye del cálculo de la pose'],correct:0,
 ...
 {tag:'Hand Grab Interactable',title:'Parámetros del Hand Grab Interactable',
  ...
  mc:['Cuántos interactores pueden agarrar el objeto a la vez','Si el agarre es por Pinch (pellizco), Palm (palma) o ambos','Qué dedos son requeridos u opcionales en el agarre','Cuándo se libera el objeto: al soltar todos o cualquier dedo'],correct:1,
...
,
 ...
 {tag:'Locomoción',title:'¿Qué es la locomoción?',
  ...
  mc:['El conjunto de sistemas y mecánicas que permiten al jugador desplazarse por el entorno evitando el mareo (teleport, snap turn, slide)','El sistema que renderiza las manos y los modelos de los joysticks','La técnica de agarrar objetos a distancia con un rayo','El componente que sincroniza el audio espacial con el giro de la cabeza'],correct:0,
...
{id:'iluminacion',name:'Iluminación y Optimización',icon:'💡',color:'#EBD23C',desc:'Draw calls, batching, lightmaps, UV2, shadow maps, LOD, pooling, overdraw',qs:[
 ...
 {tag:'Batching',title:'Batching (Static / Dynamic / GPU Instancing)',
  ...
  mc:['Reutilizar objetos pre-creados activándolos y desactivándolos','Agrupar objetos con el mismo material en una draw call; Static, Dynamic y GPU Instancing','Renderizar versiones con menos polígonos según la distancia','Comprimir las texturas a un atlas para ahorrar memoria de video'],correct:1,
 ...
 {tag:'Lightmap',title:'Lightmap',
  ...
  mc:['Una textura con la profundidad de la escena vista desde la luz','Una textura con iluminación pre-calculada aplicada a superficies estáticas vía UV2','Puntos que capturan luz bakeada para los objetos dinámicos','Un mapa que define el relieve de la superficie sin geometría extra'],correct:1,
 ...
 {tag:'Light Probes',title:'Light Probes',
  ...
  mc:['Luces puntuales en tiempo real ubicadas por la escena','Puntos que capturan luz bakeada para iluminar objetos dinámicos interpolando entre probes','Texturas de iluminación aplicadas a las superficies estáticas','Sombras suaves calculadas por frame para objetos en movimiento'],correct:1,
 ...
 {tag:'Shadow map',title:'Shadow map',
  ...
  mc:['Una textura con la profundidad vista desde la luz, para decidir las sombras','Una textura de iluminación bakeada sobre las superficies estáticas','Un mapa que guarda el relieve fino de la superficie','Una agrupación de luces para reducir las draw calls'],correct:0,
 ...
 {tag:'LOD',title:'LOD (Level of Detail)',
  ...
  mc:['Versiones del mismo objeto con distintos polígonos según la distancia al jugador','Un sistema que ajusta la iluminación según la distancia a la luz','La carga progresiva de texturas según el nivel de mip seleccionado','El descarte de objetos que quedan fuera del campo de visión'],correct:0,
...
{id:'gamefeel',name:'Game Feeling y Feedback',icon:'🎮',color:'#D43C9E',desc:'Game feeling, feedback +/−, reticles, tangibilidad y planificación VR',qs:[
 {tag:'Game Feeling',title:'Game Feeling',
  ...
  mc:['El conjunto de consecuencias visibles que produce cada acción','El grado de dificultad ajustado al nivel del jugador','La calidad de las sensaciones transmitidas durante la interacción','La estabilidad de los FPS por encima de 90 Hz'],correct:2,
 ...
 {tag:'Tangibilidad',title:'Tangibilidad del Trabajo',
  ...
  mc:['Que la calidad de las sensaciones al interactuar sea inmediata','Que las acciones produzcan consecuencias visibles y comprensibles, con impacto percibido','Que la escena tenga simulación física en la mayor cantidad de objetos','Que el usuario manipule objetos físicos reales además del visor'],correct:1,
 ...
 {tag:'Reticles',title:'Reticles',
  ...
  mc:['Para representar las manos virtuales del jugador en la escena','Para comunicar visualmente si una superficie/objetivo es válido para una acción','Para marcar el punto exacto donde mira el usuario','Para mostrar el rayo del control independientemente de la validez'],correct:1,
...
{id:'interfaces',name:'Interfaces UI en VR',icon:'🖥️',color:'#2FB57A',desc:'Canvas World Space, Add Ray/Poke, wizard del SDK, reticle y layout',qs:[
 ...
 {tag:'Add Ray Interaction',title:'Add Ray Interaction sobre un Canvas',
  body:`Clic derecho en el Canvas → <code>Interaction SDK &gt; Add Ray Interaction</code>. Abre una ventana con botones <strong>Fix</strong>: 1) convierte el Canvas a World Space, 2) agrega el módulo de interacciones a la escena. Después, <strong>Create</strong> genera el Game Object con todos los componentes para interactuar con ese Canvas desde lejos con el rayo del control.`,
  q:'¿Qué hace "Add Ray Interaction" sobre un Canvas?',
  mc:['Habilita la interacción por toque de dedo sobre el Canvas','Configura (vía Fix + Create) todo lo necesario para interactuar con el Canvas desde lejos con el rayo','Convierte el Canvas a Screen Space y le agrega un Graphic Raycaster','Agrega un Rigidbody y colliders de física al Canvas'],correct:1,
 ...
 {tag:'Reticle Data Teleport',title:'Reticle Data Teleport',
  ...
  mc:['Calcula la curva y el alcance del rayo de teletransporte','Maneja el indicador visual de teletransporte con estados válido/inválido','Define las zonas del piso por las que se puede caminar','Renderiza la malla de las manos virtuales del jugador'],correct:1,
...
{id:'building_blocks',name:'Building Blocks',icon:'📦',color:'#E8822E',desc:'Componentes modulares del Meta XR SDK: cámara, manos, grab, ray, teleport, eye gaze y passthrough',qs:[
 ...
 {tag:'Camera Rig',title:'Camera Rig',
  ...
  mc:['Solo la cámara con renderizado estereoscópico, sin tracking','La cámara, los controladores y el tracking de cabeza y cuerpo','El sistema de sonido espacial y la mezcla por capas','El conjunto de manos virtuales y sus animaciones'],correct:1,
 ...
 {tag:'Hand Tracking',title:'Hand Tracking',
  ...
  mc:['Dibuja una malla hiperrealista de las manos','Detecta la posición y poses de las manos sin controles','Anima las manos virtuales a partir de los controles','Detecta micro-gestos del pulgar sobre el índice'],correct:1,
 ...
 {tag:'Real Hands',title:'Real Hands',
  body:`Muestra las <strong>manos reales del usuario mediante passthrough</strong> (el video de las cámaras del visor) en vez de manos virtuales renderizadas. Aporta máxima presencia —ves tus propias manos— y se apoya en el passthrough de realidad mixta.`,
  q:'¿Qué caracteriza a las Real Hands?',
  mc:['Muestran las manos reales del usuario por passthrough, en vez de manos renderizadas','Son una malla virtual con más polígonos que las Virtual Hands','Dependen exclusivamente del hand tracking para funcionar','Reemplazan al Camera Rig en escenas de realidad mixta'],correct:0,
  why:'Real Hands = las manos reales del usuario mostradas por passthrough (video de las cámaras), no una malla renderizada.'},
 ...
 {tag:'Ray Interaction',title:'Ray Interaction',
  ...
  mc:['Para agarrar objetos al tocarlos directamente con la mano','Para interactuar a distancia con botones lejanos o UI','Para encajar objetos en posiciones predefinidas','Para detectar hacia dónde mira el usuario'],correct:1,
...
,
 {tag:'Locomotion Controller Group',title:'Locomotion Controller Group',
  ...
  mc:['Un agrupador/handler que administra las reglas de movimiento (Teleport, Snap Turn, Slide) y las comunica a la cápsula física del jugador','La zona del piso donde el jugador puede teletransportarse','El prefab que muestra los modelos 3D de los joysticks físicos','El componente que define las poses de los dedos al agarrar un objeto'],correct:0,
...
{id:'event_wrappers',name:'Event Wrappers',icon:'🔗',color:'#7A33B5',desc:'Enlazar acciones sin código: cada wrapper expone los eventos del interactor o interactable en el Inspector',qs:[
 ...
 {tag:'Interactor Wrapper',title:'Interactor Unity Event Wrapper',
  ...
  mc:['A los del interactable: el objeto que recibe la acción','A los del interactor: la mano o el control','A los eventos de puntero de poke o ray sobre cualquier objeto','A los eventos de encaje de un objeto en un slot'],correct:1,
 ...
 {tag:'Interactable Wrapper',title:'Interactable Unity Event Wrapper',
  ...
  mc:['Desde la mano o el control','Desde el objeto: cuándo es hovereado o seleccionado','Desde la cámara, según hacia dónde mira el jugador','Desde el sistema de teletransporte y sus reticles'],correct:1,
 ...
 {tag:'Pointable Wrapper',title:'Pointable Unity Event Wrapper',
  ...
  mc:['Es específico del agarre con la mano','Es el más general: eventos de puntero (hover/select/move) para poke o ray','Responde solo a los eventos del interactor (mano/control)','Es exclusivo del toque con el dedo sobre botones'],correct:1,
 ...
 {tag:'Hand Grab Wrapper',title:'Hand Grab Unity Event Wrapper',
  ...
  mc:['Los de tocar un botón o superficie con el dedo','Los de agarrar y soltar un objeto con la mano','Los de apuntar y seleccionar de lejos con el rayo','Los de encajar un objeto en un slot'],correct:1,
 ...
 {tag:'Poke Wrapper',title:'Poke Unity Event Wrapper',
  ...
  mc:['Al agarrar y soltar un objeto con la mano','Al tocar con el dedo una superficie o botón','Al apuntar de lejos con el rayo del control','Al detectar un micro-gesto del pulgar sobre el índice'],correct:1,
 ...
 {tag:'Micro Gesture Wrapper',title:'Micro Gesture Unity Event Wrapper',
  ...
  mc:['Grandes movimientos del brazo y la muñeca','Micro-gestos del pulgar sobre el índice (swipes/taps)','El encaje de un objeto en un slot','La orientación de la cabeza dentro del espacio'],correct:1,
...
const _flujosIntegral = [
 {t:'Sonido al agarrar con las manos',prompt:'Ordená el flujo para que suene un feedback al agarrar un objeto con las manos, sin programar.',steps:['Agregar Rigidbody y Collider al objeto','Agregar el componente Grabbable','Agregar Hand Grab Interactable','Agregar un AudioSource con el clip','Agregar Pointable Unity Event Wrapper y referenciar el Grabbable','En el evento When Select, llamar a AudioSource.Play()'],why:'El objeto se hace agarrable (Rigidbody + Grabbable + Hand Grab Interactable) y el Pointable Unity Event Wrapper (referenciando el Grabbable) dispara el AudioSource en When Select. Cero código.'},
...
async function scoreVF(q,marks,just){
  ...
  for(let k=0;k<tot;k++){
    ...
    try{
      ...
      // lenient: alcanza con capturar la idea; explayarse no penaliza (una justificación no vacía nunca baja de 0.6)
      sum+=sim>=0.45?1:(sim>=0.3?0.75:(j?0.6:0.5));
    }catch(e){sum+=j?1:0.5;}
  }
...
const FLOW_REASONS={
 'Agregar funcionalidades con Building Blocks':[
  `Abrís el panel primero: es de donde salen todos los módulos; sin abrirlo no podés arrastrar nada.`,
  `El Camera Rig antes que todo porque trae la cámara y el OVR Manager. Si agregaras las manos o el objeto sin el rig, no habría jugador ni tracking al que engancharlos.`,
  `El Passthrough va sobre el rig ya puesto: necesita la cámara del rig para mostrar el mundo real. Solo si querés Realidad Mixta.`,
  `El Hand Tracking después del rig porque usa las cámaras del visor (que trae el rig) para detectar las manos; sin rig no hay de dónde leerlas.`,
  `Las Synthetic Hands después del tracking: son solo lo visual y se mueven con los datos del Hand Tracking. Sin el tracking puesto, no tendrían qué las anime.`,
  `El objeto agarrable al final porque para agarrarlo ya tienen que existir las manos/interactores. Antes de eso sería un objeto que nada puede tomar.`],
 'Hacer un objeto agarrable con el Grab Wizard':[
  `Primero el cubo: el wizard actúa sobre un objeto que ya existe; sin objeto no hay a qué agregarle el agarre.`,
  `Abrís el Grab Wizard sobre ese cubo: es el asistente que va a armar la interacción.`,
  `Elegís interactor y gesto antes de crear porque el wizard necesita saber con qué se agarra (manos/controles) para generar los componentes correctos.`,
  `Fix va antes de Create porque el agarre necesita un Rigidbody y Fix lo agrega. Si dieras Create sin Rigidbody, la interacción quedaría incompleta.`,
  `Create recién después de Fix: ahí inyecta todos los componentes, ya con la física resuelta.`,
  `Ajustar y probar al final: solo tiene sentido cuando la interacción ya está creada y andando.`],
 'Hacer un objeto agarrable paso a paso (manual)':[
  `Armás la jerarquía primero porque cada componente que sigue vive en un hijo distinto (Visual, Colliders, Hand Grab, Controller Grab); sin esos lugares, después no tenés dónde ponerlos.`,
  `El Rigidbody antes que el Grabbable porque el Grabbable, al agregarlo, te pide una referencia a un Rigidbody: si todavía no existe, no hay qué asignarle.`,
  `El Grabbable después del Rigidbody: es lo que mueve el objeto usando esa física. Sin él, el Rigidbody cae pero nada lo puede agarrar.`,
  `Los Colliders recién ahora: definen DÓNDE la mano detecta el agarre. Antes de que el objeto sea agarrable no tienen nada que activar.`,
  `El Hand Grab Interactable después de los colliders porque referencia el Grabbable y el Rigidbody ya creados y usa el collider como zona de contacto; sin ellos no tendría a qué apuntar.`,
  `El Controller Grab al final: misma lógica pero para controles; se suma sobre el objeto ya agarrable, no lo reemplaza.`],
 'Sonido al agarrar y soltar (Pointable Event Wrapper)':[
  `El Pointable Wrapper primero porque es quien expone los eventos Select/Release del objeto: sin él no habría eventos a los que enganchar el sonido.`,
  `Creás los vacíos Select/Release antes de los audios para tener dónde colgar cada Audio Source por separado.`,
  `Play On Awake OFF antes de conectar: si quedara ON, el sonido sonaría solo al iniciar la escena en vez de al agarrar/soltar.`,
  `Asignás los clips antes de conectar los eventos: el evento va a llamar al Audio Source, que ya debe tener su sonido cargado.`,
  `When Select → Play: recién con wrapper, vacíos y clips listos podés enganchar el evento de agarrar.`,
  `When Release → Play al final: mismo esquema para soltar, una vez que lo de agarrar ya quedó armado.`],
 'Manos y controles simultáneos (multimodal)':[
  `Arrancás de una escena limpia (sin Main Camera) porque el Camera Rig trae su propia cámara; si dejás la vieja, chocan dos cámaras.`,
  `Agregás los Building Blocks antes de configurar nada: son la base (rig, tracking, manos, grab) sobre la que se activa el modo mixto.`,
  `El OVR Manager antes que los ajustes de mano porque ahí se habilita "Simultaneous Hands and Controllers"; sin eso activado, los pasos siguientes no tienen efecto.`,
  `Show State = Always después de habilitar el modo: hace que la mano no desaparezca al tomar el control. Depende de que el modo simultáneo ya esté prendido.`,
  `Enlazás la mano sintética con el data source del control recién ahora: necesitás el modo activo y el Show State puesto para que la mano siga al joystick.`,
  `El prefab de controles sintéticos al final, también en Always, para cerrar la convivencia mano+control sobre todo lo ya configurado.`],
 'Crear una pose de agarre (Hand Grab Pose)':[
  `Add Hand Grab Pose primero porque la pose se genera desde el Hand Grab Interactable ya existente: sin él no hay de dónde crearla.`,
  `Abrís el hijo generado porque es el objeto concreto que vas a mover; no podés editar lo que todavía no se creó.`,
  `Posicionás/rotás la mano antes de tocar los dedos: primero dónde va la mano y, sobre esa ubicación, después afinás cada dedo.`,
  `Ajustás los dedos después de ubicar la mano porque doblarlos antes, con la mano mal puesta, te obligaría a rehacerlo.`,
  `Configurás mano y Fingers Freedom al final para fijar qué queda libre/bloqueado una vez que la pose ya calza.`],
 'Crear la pose espejo para la otra mano':[
  `Create Mirror primero porque parte de una pose ya hecha: la refleja en vez de rehacerla dedo por dedo. Si no hubiera pose, no habría qué espejar.`,
  `Verificás el objeto "mirror" antes de tocar nada: confirmás que se creó bien y ya quedó configurado para la otra mano.`,
  `Ajustás la rotación solo al final y solo si no calza: es el único retoque que a veces deja pendiente el espejado.`],
 'Agregar múltiples puntos de agarre a un objeto':[
  `Creás un vacío nuevo primero porque cada punto de agarre extra es un objeto aparte donde irá su interactable.`,
  `Le ponés otro Hand Grab Interactable: cada zona de agarre necesita el suyo; uno solo no puede cubrir dos puntos.`,
  `Verificás que referencie el mismo Grabbable/Rigidbody porque todos los puntos mueven el MISMO objeto; si apuntaran a otro, se partiría el comportamiento.`,
  `Agregás y posicionás la pose sobre la nueva zona una vez que el interactable ya existe y referencia bien.`,
  `Pose espejo y colliders al final: los colliders deben cubrir cada zona o el agarre nuevo no se detecta.`],
 'Configurar el Snap Interactor en el objeto que se mueve':[
  `Is Trigger en el collider primero porque el Snap detecta por solapamiento (trigger); sin eso, el collider chocaría físicamente en vez de dejar encajar.`,
  `Creás el vacío Snap Interactor dentro del objeto que se mueve: es el objeto el que "busca" dónde encajar.`,
  `Le ponés el componente Snap Interactor antes de las referencias, para tener dónde asignarlas.`,
  `Referenciás el Grabbable como Pointable Element porque el Snap necesita saber cuándo lo agarrás/soltás para decidir el encaje; sin esa referencia no se entera.`,
  `Referenciás el Rigidbody al final: es lo que el Snap mueve al encajar. Sin él no puede reposicionar el objeto.`],
 'Crear el Snap Interactable (destino del Snap)':[
  `Creás y ubicás el vacío primero porque el destino tiene que estar donde querés que el objeto encaje.`,
  `Box Collider antes del componente: define la zona que activa el Snap; sin zona, el interactable no sabría dónde detecta.`,
  `Rigidbody con Use Gravity off + Is Kinematic antes de terminar: si tuviera física normal, el destino se caería o lo empujaría el objeto al encajar (es el bug del orbitado).`,
  `Recién ahí el Snap Interactable: se apoya en la zona (collider) y en el rigidbody kinemático ya configurados.`],
 'Crear el visual de la zona del Snap':[
  `Creás el cubo visual primero porque es lo que vas a mostrar/ocultar; sin él no hay nada que controlar.`,
  `Material transparente verde antes de conectar eventos: define que sea una guía y no tape la escena.`,
  `Le sacás el collider al cubo visual porque, si lo dejás, interferiría con la detección del Snap; el visual no debe chocar con nada.`,
  `Interactable Unity Event Wrapper referenciando el Snap Interactable: sin esa referencia no recibiría los eventos de encaje.`,
  `When Select → SetActive(false): oculta el visual cuando el objeto encaja (ya no hace falta la guía).`,
  `When Unselect → SetActive(true) al final: lo vuelve a mostrar al sacar el objeto; es el par del evento anterior.`],
 'Varios Snaps con punto por defecto y retorno por tiempo':[
  `Duplicás los Snaps primero porque no podés configurar "default" ni "a cuál vuelve" si todavía no existen varios destinos.`,
  `Abrís Options del Snap Interactor: es el panel desde donde se referencian esos Snaps.`,
  `Snap Pose Transform antes de los demás: define el pivote con el que el objeto se acomoda al encajar.`,
  `Default Interactable: el Snap donde arranca el objeto; tiene que existir, por eso duplicaste antes.`,
  `Time Out Interactable: a cuál Snap vuelve solo; otra referencia a un destino ya creado.`,
  `Time Out al final: los segundos de espera; el "cuánto" se define después del "a dónde vuelve".`],
 'Filtrar con qué Snaps encaja el objeto (Tags)':[
  `Tag Set en los Snaps primero porque tenés que etiquetar los destinos antes de poder filtrarlos: sin etiquetas no hay por qué filtrar.`,
  `Asignás una etiqueta concreta a cada Snap (cube/sphere) para poder distinguirlos.`,
  `Tag Set Filter en el interactor después: es el objeto móvil el que decide con qué tags encaja.`,
  `Definís Required/Excluded Tags: la regla en sí; necesita que las etiquetas ya existan en los Snaps.`,
  `Referenciás el filtro en Interactable Filters al final, o el interactor no lo aplica aunque esté configurado.`],
 'Crear un Canvas y habilitar interacción (Ray y Poke)':[
  `Creás el contenedor UI primero para ordenar la jerarquía del Canvas.`,
  `Creás el Canvas dentro: es sobre lo que después corren Ray y Poke.`,
  `Add Ray + Fix antes de Poke porque Fix pasa el Canvas a World Space (obligatorio en VR); sin eso, los rayos del SDK no lo detectan.`,
  `Elegís manos/controles y creás: define quién interactúa a distancia con el Canvas ya en World Space.`,
  `Add Poke al final: suma el toque directo con el dedo. Es un interactable aparte del Ray, por eso se agrega después.`],
 'Crear una zona de teleport con NavMesh':[
  `Creás el vacío y Reset del Transform primero para partir de una posición limpia (0,0,0).`,
  `Teleport Interactable: marca ese objeto como zona de destino; es la pieza que después usará el NavMesh.`,
  `NavMesh Surface + config del agente antes del bake: definís qué se considera piso caminable.`,
  `Bake antes de referenciar porque genera la superficie navegable; si referenciás sin hornear, no hay zona calculada.`,
  `Referenciás el NavMesh Surface en el Teleport Interactable recién con el bake hecho.`,
  `Reticle Data Teleport al final: define qué se muestra al final del rayo (válido/inválido), una vez que la zona existe.`],
 'Crear una zona de teleport con Collider (escalera)':[
  `Creás el vacío y Reset del Transform para arrancar limpio.`,
  `Teleport Interactable con Collider Surface: en superficies inclinadas (escalera) el NavMesh no calza bien, por eso elegís collider.`,
  `Creás el vacío con Box Collider que represente la escalera: es la superficie concreta a la que teletransportarte.`,
  `Alineás el collider con la escalera antes de referenciarlo: si queda torcido, aterrizarías donde no va.`,
  `Referenciás ese Collider en el Collider Surface, o la zona no funciona aunque el collider exista.`,
  `Reticle Data Teleport al final, para mostrar el destino al apuntar.`],
 'Crear una zona de teleport inválida':[
  `Creás el vacío y Reset del Transform para arrancar limpio.`,
  `Teleport Interactable con Allow Teleport OFF: es lo que define que ahí NO se puede caer (una zona igual a las otras pero bloqueada).`,
  `Bajás el Score en Options porque, si dos zonas se superponen, gana la de mayor score; con score bajo esta pierde y no roba destinos válidos.`,
  `Agregás y referenciás una Surface horizontal para cubrir el área que querés bloquear.`,
  `Reticle Data Teleport en modo inválido al final: muestra el rayo en rojo al apuntar, avisando que no vale.`],
 'Teleport a un punto fijo (hotspot)':[
  `Creás el GameObject donde querés caer: el hotspot es un destino fijo y necesita un lugar concreto.`,
  `Add Teleport Interaction sobre él arma la interacción sobre ese punto.`,
  `Se genera el Hotspot: el destino absoluto queda configurado (no lo hacés a mano).`,
  `El Teleport Interactor (en la mano) apunta a ese hotspot; sin destino creado, no tendría a qué apuntar.`,
  `El reticle confirma y al soltar te lleva: como es salto instantáneo y no movimiento continuo, no marea.`],
 'Girar de a pasos (snap turning)':[
  `Partís de un rig con locomoción ya configurada porque el giro se apoya en ese sistema; sin él no hay a qué sumarle el turner.`,
  `Ubicás el Controller Turner Interactor en el control: es quien genera el input de giro.`,
  `En el Turner Event Broadcaster elegís modo Snap: convierte ese input en giros por pasos (necesita el interactor que le manda el input).`,
  `Seteás los grados del giro (ej. 45) una vez elegido el modo Snap.`,
  `Probás al final: verificás que el giro por pasos marea menos que el continuo.`],
 'Bakear la iluminación del interior':[
  `Marcás Static (Contribute GI) primero porque solo los objetos marcados entran al bake; si no lo hacés, quedan fuera de la luz horneada.`,
  `Confirmás Generate Lightmap UVs en los FBX antes de bakear: sin un UV2 sin solapamientos, la luz de una cara pisa a la otra y sale mal.`,
  `Configurás las luces como Baked/Mixed para que se pre-calculen; si quedaran Realtime, no se hornearían.`,
  `Guardás la escena antes de bakear porque bakear sin guardar suele dar resultados raros.`,
  `Generate Lighting al final: recién ahí se hornea todo lo preparado en los pasos anteriores.`]
};
